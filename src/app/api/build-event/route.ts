...
export async function POST(req: NextRequest) {
  try {
    ...
    const updated = updateActiveBuild({
      ...
      environment: body.environment || store.activeBuild?.environment || "crave",
      craveJobId: body.craveJobId || store.activeBuild?.craveJobId,
      craveUrl: body.craveUrl || store.activeBuild?.craveUrl,
      endTime: body.status === "success" || body.status === "failed" ? new Date().toISOString() : undefined,
      duration,
      artifact: body.artifact !== undefined ? body.artifact : store.activeBuild?.artifact,
      artifactSize: body.artifactSize !== undefined ? body.artifactSize : store.activeBuild?.artifactSize,
      artifactSha256: body.artifactSha256 !== undefined ? body.artifactSha256 : store.activeBuild?.artifactSha256,
      errorLog: body.errorLog !== undefined ? body.errorLog : store.activeBuild?.errorLog,
